...
(function() {
  ...
  preload_frame = function(parent, animation, time) {
    ...
    for (_i = 0, _len = additions.length; _i < _len; _i++) {
      ...
      img = $('<img></img>');
      img.attr('id', 'pterodactyl_' + time + '_part_' + part_id);
      parent.append(img);
      img.addClass('pterodactyl_' + time);
      ...
      if (images != null) {
        ...
        img.attr('src', 'img/' + directory + 'images' + divider + image_id);
      }
    ...
    for (_j = 0, _len2 = morph_starts.length; _j < _len2; _j++) {
      ...
      _results.push(image_id_2 != null ? (image_id != null ? (img_tween = $('<img></img>'), img_tween.attr('id', 'pterodactyl_' + time + '_part_' + part_id + '_tween'), parent.append(img_tween), img_tween.addClass('part'), img_tween.addClass('pterodactyl_' + time), img_tween.css(browser_prefix + 'transform-origin', '0 0'), img_tween.css({
        display: 'none',
        position: 'absolute'
      }), tweens != null ? (tween = tweens[[part_id, image_id, image_id_2]].image, img_tween.attr('src', tween.get(0).toDataURL())) : img_tween.attr('src', directory + 'parts' + divider + part_id + divider + 'srcimages' + divider + image_id + divider + 'dstimages' + divider + image_id_2), set_transform(img_tween, matrix), enable_transition(img_tween)) : void 0, img_2 = $('<img></img>'), img_2.attr('id', 'pterodactyl_' + time + '_part_' + part_id + '_2'), img_2.addClass('pterodactyl_' + time), parent.append(img_2), img_2.css(browser_prefix + 'transform-origin', '0 0'), img_2.css({
        position: 'absolute',
        opacity: 0,
        display: 'none'
      }), images != null ? (image_2 = images[image_id_2], img_2.attr('src', image_2.get(0).toDataURL())) : img_2.attr('src', 'img/' + directory + 'images' + divider + image_id_2), set_transform(img_2, matrix_2), enable_transition(img_2)) : void 0);
    }
    return _results;
  };
  prepare_frame = function(parent, animation, time) {
    ...
    for (_i = 0, _len = additions.length; _i < _len; _i++) {
      _ref2 = additions[_i], part_id = _ref2.part_id, image_id = _ref2.image_id, matrix = _ref2.matrix;
      img = $('#pterodactyl_' + time + '_part_' + part_id);
    ...
    for (_l = 0, _len4 = morph_starts.length; _l < _len4; _l++) {
      ...
      if (image_id_2 != null) {
        if (image_id != null) {
          img_tween = $('#pterodactyl_' + time + '_part_' + part_id + '_tween');
          img_tween.attr('id', 'animpart_' + part_id + '_tween');
          img_tween.css({
            display: 'inline'
          });
        }
        img_2 = $('#pterodactyl_' + time + '_part_' + part_id + '_2');
  ...
  play_helper = function(params) {
    ...
    ($(".pterodactyl_wrapper")).remove();
    parent = $("<div></div>");
    parent.addClass("pterodactyl_wrapper");
    wid = container.width();
  ...
  pterodactyl_player.play = function(params, container) {
    var url;
    stopped = false;
  ...
  return pterodactyl_player.stop = function() {
    return stopped = true;
  };
})();

$(function() {
    pterodactyl_player.play('pterodactyl');
});
